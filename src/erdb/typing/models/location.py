...
class LocationDetail:
    summary: str = Field(
        ...
        min_length=1,
        pattern=r"^.*[^\\]\.?$",  # Negative lookbehind not supported
        example="found on top a giant spear in Leyndell, Royal Capital",
    )
    quantity: PositiveInt | Literal["infinite"] | None = Field(
        None,
    ...
    directions: NonEmptyStr | None = Field(
        None,
    ...
    price_bought: PositiveInt | None = Field(
        None,
    ...
    requirements: list[NonEmptyStr] | None = Field(
        None,
    ...
    blockers: list[NonEmptyStr] | None = Field(
        None,
